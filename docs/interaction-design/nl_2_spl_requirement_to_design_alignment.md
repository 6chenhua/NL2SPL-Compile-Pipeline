...
| kind | 触发条件 |
|---|---|
| `missing_handler` | 已知 failure/exception condition，但缺少 handler action |
| `missing_trigger` | 已知 failure handling 或 handler candidate，但不清楚由哪一步触发 |
| `missing_output_producer` | required output 已声明，但没有 producer step |
| `type_or_contract_ambiguity` | 变量类型、worker IO、handoff、API 契约不清楚或冲突 |
...
| 用户只说 handle failures properly | 不生成具体 EXCEPTION_FLOW；输出 underspecified warning 或 suggestion |
| 用户列出 failure condition | 生成 partial EXCEPTION_FLOW condition；标注 missing_handler |
| 用户列出 condition + handler | 生成完整 EXCEPTION_FLOW + COMMAND |
| 用户列出 handler 但 condition 不清楚 | 不生成完整 exception flow；输出 missing_trigger |
...
1. 有明确 `trace_refs`；
2. 其行为语义不是纯 `assumed`；
3. 如果是 `INPUT`，原文必须明确要求询问用户，或有明确缺失项且系统允许提出问题；
...
3. 多轮交互 UI；
4. 自动把用户未确认的 assumption 写回 SPL；
5. 复杂 semantic drift 自动检测；
6. 多 schema 冲突处理；
7. 完整用户补齐闭环。
...
4. failure trigger 不明确时，输出 missing_trigger diagnostic。
5. required output 能进入 Worker outputs。
6. required output 没有 producer step 时，输出 missing_output_producer diagnostic。
7. 变量/契约存在类型或结构不明确时，输出 type_or_contract_ambiguity diagnostic。
...
| 用户列出 condition + handler | 生成完整 EXCEPTION_FLOW + COMMAND |
| 用户只说 Handle failures properly | 不生成具体 exception flow，输出 underspecified warning |
...
| child worker 缺 IO/handoff | 不渲染 INVOKE_WORKER，输出 diagnostic |
| 用户未提 API | 不生成 API/CALL_API |
| 用户提 connector/source repository | 可生成 integration/resource hint |
| assumed behavior step | 不渲染，进入 assumptions/report |
