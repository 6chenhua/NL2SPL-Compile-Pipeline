...
class TestIRNormalizer:
    ...
    def test_pruned_step_variable_constraint_target_does_not_block(self) -> None:
        """Constraints pointing at pruned orphan step vars stay reportable."""
        normalizer = IRNormalizer()
        resources = ResourceRegistryIR(
            variables=[
                VariableSpec(
        ...
        constraints = [
            ConstraintIR(
                ...
                "Missing timeframe",
                "requirement",
                targets=["variable:missing_timeframe"],
                source_span_ids=["s1"],
            )
        ]
        ...
        assert not any("missing_timeframe" in error for error in errors)
        assert any("Pruned unused step variable: missing_timeframe" in w for w in warnings)
