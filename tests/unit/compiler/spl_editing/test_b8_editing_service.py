...
class _StubContextBuilder:
    def build(self, issue, target, snapshot, instruction=None):
        return RepairContext(issue=issue, target=target, user_instruction=instruction)
...
class _StubHandler:
    def generate_suggestions(
        ...
        entries,
        instruction=None,
        selected_patch_types=None,
        *,
        rendered_user_prompt=None,
        selectable_refset=None,  # R6
        catalog_entry=None,  # R6
    ):
        return (
            RepairSuggestion(
                suggestion_id="sug_0",
...
class _StubApplier:
    def apply(self, patch, snapshot):
        ...
        patched = snapshot.derive(
            next_token,
            worker_step_plan=sp,
            worker_block_plan=bp,
            compile_diagnostics=tuple(
                d
                for d in snapshot.compile_diagnostics
...
class TestB8ApplyVerify:
    def test_non_materialized_dict_apply_rejected(self) -> None:
        """R11: AddExceptionHandlerStep dict suggestions cannot apply through runtime appliers."""
        svc = _make_service()
        issue = _make_mh_issue()
        snap = ArtifactSnapshot("snap_1", "run_1", 0)
        svc.register_compile_result(snap)
        session = svc.create_session("run_1", issue)
        suggestions = svc.generate_suggestions(session.session_id)
        with pytest.raises(SPLEditingError, match="materialization plan"):
    ...
    def test_cross_session_apply_rejected(self) -> None:
        ...
        issue_b = EditableIssue(
            issue_id="i2",
            primary_diagnostic_id="d2",
            related_diagnostic_ids=("d2",),
            issue_group_id=None,
            kind="missing_handler",
    ...
    def test_apply_rejects_unmaterialized_add_exception_suggestion(self) -> None:
        svc = _make_service()
        issue = _make_mh_issue()
        diag = CompileDiagnostic(
            "d1",
    ...
    def test_direct_runtime_validator_not_used_for_materialized_patch_types(self) -> None:
        # R11: AddExceptionHandlerStep must be a ConstructRepairIntent handled by
        # RepairMaterializationService. Dict payloads fail before legacy validators.
        from nl2spl.compiler.spl_editing.core.model import PatchTypeContract
