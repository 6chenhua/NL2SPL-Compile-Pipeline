...
def _target(**kw: object) -> RepairTarget:
    d: dict[str, object] = dict(
        target_ref="x", target_kind="EXCEPTION_FLOW",
        irs_ref=DiagnosticIRSRef(construct_type="EXCEPTION_FLOW",
                                  construct_id="x", slot_name="handler_action"),
        affordance_id="exception_flow.add_handler_step", construct_path=(),
        worker_id="w_main",
    )
    d.update(kw)
...
class TestB3ContextBuilders:
    ...
    def test_worker_promotion_context(self) -> None:
        ctx = WorkerPromotionContextBuilder().build(
            _issue(kind="type_or_contract_ambiguity",
        ...
        ctx = WorkerHandoffContextBuilder().build(
            _issue(kind="type_or_contract_ambiguity",
                   ...
                   irs_ref=DiagnosticIRSRef(
                       ...
                       construct_id="x", slot_name="target")),
