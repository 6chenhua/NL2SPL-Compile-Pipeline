...
def _b7c_patch(**kw: object) -> RepairPatch:
    d: dict[str, object] = dict(
        ...
        base_compile_run_id="run_1", artifact_snapshot_id="snap_1",
        overlay_version=0, verification_lane="A",
        payload={"worker_id": "w_main", "prompt_text": "Ask user.",
...
class TestB7bConvertToMainFlow:
    ...
    def test_wrong_construct_type_rejected(self) -> None:
        with pytest.raises(PatchValidationError, match="construct_type"):
            ConvertDelegationToMainFlowStepValidator().validate(
                _b7b_patch(irs_ref=DiagnosticIRSRef(
...
class TestB7cIdentity:
    ...
    def test_empty_construct_id_rejected(self) -> None:
        with pytest.raises(PatchValidationError, match="construct_id"):
            ConvertDelegationToRequestInputValidator().validate(
                _b7c_patch(irs_ref=DiagnosticIRSRef(
                    construct_type="WORKER_PROMOTION",
                    construct_id="", slot_name="promotion_input_contract",
                )), _snap())
    ...
    def test_target_ref_mismatch_rejected(self) -> None:
        with pytest.raises(PatchValidationError, match="target_ref"):
            ConvertDelegationToRequestInputValidator().validate(
                _b7c_patch(target_ref="worker_promotion:wrong_id"),
                _snap())

    def test_applier_creates_step_with_marker(self) -> None:
        snap = _snap()
        patched, _ = ConvertDelegationToMainFlowStepApplier().apply(
            _b7b_patch(), snap)
        step = patched.worker_step_plan.worker_steps["w_main"][0]
        assert step.metadata.get("resolution_kind") == "converted_to_main_flow_step"

    def test_verifier_finds_step(self) -> None:
        verifier = ConvertDelegationToMainFlowStepVerifier()

        class FakeGated:
            steps = [StepIR("st_x", "Do", [], "GENERAL_COMMAND",
                             metadata={
        ...
        assert verifier.verify(
            _b7b_patch(), _snap(), _snap(), FakeArtifacts()) == ()
...
class TestB7cConvertToRequestInput:
    ...
    def test_missing_value_target_rejected(self) -> None:
        with pytest.raises(PatchValidationError, match="value_target"):
            ConvertDelegationToRequestInputValidator().validate(
                _b7c_patch(payload={"worker_id": "w_main",
                                     "prompt_text": "Ask."}), _snap())
    ...
    def test_applier_creates_request_input_step(self) -> None:
        snap = _snap()
        patched, _ = ConvertDelegationToRequestInputApplier().apply(
            _b7c_patch(), snap)
        step = patched.worker_step_plan.worker_steps["w_main"][0]
        assert step.command_type == "REQUEST_INPUT"
        assert step.metadata.get("value_target") == "user_input"
        assert step.metadata.get("resolution_kind") == "converted_to_request_input"

    def test_verifier_finds_step(self) -> None:
        verifier = ConvertDelegationToRequestInputVerifier()

        class FakeGated:
            steps = [StepIR("st_x", "Ask.", [], "REQUEST_INPUT",
                             outputs=["user_input"],
                             metadata={
        ...
        assert verifier.verify(
            _b7c_patch(), _snap(), _snap(), FakeArtifacts()) == ()
