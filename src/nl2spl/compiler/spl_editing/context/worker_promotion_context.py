...
class WorkerPromotionContextBuilder(RepairContextBuilder):
    ...
    def build(
        ...
        if plan is not None:
            child_workers = [
                w.worker_id for w in plan.workers if getattr(w, "kind", None) == "child"
            ]
        ...
        if child_worker_id is not None:
            ...
            child = next(
                (
                    w
                    for w in (plan.workers if plan is not None else [])
                    if w.worker_id == child_worker_id
                ),
                None,
            )
            if child is not None:
                meta["child_input_fields"] = [field.name for field in child.input_contract]
