...
class BindExistingProducerStepValidator(PatchValidator):
    def validate(self, patch: RepairPatch, snapshot: ArtifactSnapshot) -> None:
        ...
        if target_step is None:
            raise PatchValidationError(f"Step '{step_id}' not found in worker '{worker_id}'")
        if (
            not target_step.source_span_ids
            and target_step.metadata.get("origin") != "user_confirmed_repair"
        ):
            raise PatchValidationError(
