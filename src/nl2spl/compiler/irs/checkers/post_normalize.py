...
This checker is the spec-driven replacement path for the legacy
``PostNormalizeIRSChecker``.  It consumes assembled ``WorkerIR`` through
``IRSCheckContext.normalized_ir`` and emits ``ConstructSatisfactionReport``
objects; ``DiagnosticProjector`` then produces ``CompileDiagnostic``.
"""
